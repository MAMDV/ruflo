...
| 2026-09-18 | memory | `AgentDBAdapter.store()` (default in-process write path) never checked for an existing entry under the same `(namespace,key)` before writing — a fresh random id every call left the prior occupant an orphan, unreachable via `getByKey()` but still live in `entries`/`namespaceIndex`/`tagIndex` and, for embedded entries, still a point in the HNSW index, so `search()`/`semanticSearch()` returned stale duplicates forever; fixed by evicting the prior occupant via the adapter's own existing `delete()` path before storing. Independently flagged by two prior nightly scans (09-12, 09-17) as a future-night candidate | #3351 | (pending) | yes | ACCEPT-scoped | 4 new discriminating tests, baseline-fails/candidate-passes via stash isolation; full suite 517→521 passed (1 pre-existing unrelated failure, unchanged both ways); `tsc --noEmit` clean; independent adversarial critique CONFIRMED-WITH-CAVEATS — disclosed gap: fix covers sequential re-stores only, concurrent same-key `store()` calls can still race (follow-up, not fixed tonight) | e698eaff... | 09-08..09-12 (5 nights) MERGED in a 2026-09-14/15 batch; 09-15/09-16/09-17 still open/draft, none stale (<4d old); 09-13/09-14 no-run gap noted above |

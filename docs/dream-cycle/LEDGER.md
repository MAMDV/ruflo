...
| 2026-08-20..23 | (none) | **No Dream Cycle ran** — confirmed via `git ls-remote --heads origin "dream/*"` (no branches), `search_pull_requests head:dream/2026-08-2` (0 results), `search_issues` for "Dream Cycle 2026-08-2{0,1,2,3}" (0 results). Distinct from the 08-14..08-18 ledger-append bug (that gap had real branches/PRs/issues, just no ledger row); this is a genuine pipeline-did-not-run gap, first flagged 08-24, re-confirmed 08-25/08-26/08-27, root cause still undiagnosed. | LOCAL | NONE | no | n/a | n/a | n/a | flagged 4 consecutive nights (08-24 through 08-27), not yet investigated |
| 2026-08-24 | swarm | `QueenCoordinator.weightedConsensus()` trust weights computed but structurally could not reach the vote tally (rode inertly in an opaque value blob) — threaded through propose()/vote() into raft/byzantine/gossip tally logic, [0,1]-clamped | #3085 | #3086 | yes | ACCEPT | gossip weighted-flip test: baseline `approved:false` → candidate `approved:true` (approvalRate≈0.905); 226/226 tests | 08d44c58... | MERGED 2026-09-14 |
| 2026-08-25 | performance | `HNSWIndex.Quantizer.productQuantize()/productQuantizeDistance()` correct but never called by the search path (dispatch fixed, PQ-vector normalization skipped) | #3093 | #3094 | yes | ACCEPT | recall@10: 0.097 (pre-fix) → 0.270 (post-fix) on seeded synthetic corpus; 461/462 tests (1 pre-existing unrelated env failure) | 300da158... | MERGED 2026-09-14 |
| 2026-08-26 | security | `authorizeMcpTool()` trusted unsigned `CLAUDE_FLOW_PRINCIPAL_ID` env var for caller identity (ASI07) — ADR-377's built-but-unwired Ed25519 `verifyInvocationToken` bound into the live chokepoint | #3102 | #3103 | yes | ACCEPT-with-caveats | 6/6 new deterministic auth scenarios pass post-fix, fail pre-fix; `@claude-flow/security` 583/583, `@claude-flow/codex` 241/241 | fd944eae... | awaiting CI as of 2026-09-14 |
| 2026-08-27 | intelligence | `distillLearning()`'s EWC forgetting-gate called `getPenalty([oldConf],[newConf])`, collapsing to 1 of 384 Fisher dims for every pattern — `computeConfidencePenalty`/`updateFisherFromConfidences` (already implemented, docstring'd for this exact call site, 0 prior callers) wired in | #3109 | #3110 | yes | ACCEPT-scoped | baseline: new regression test fails (methods never called); candidate: passes. 152/152 tests (+4 new), tsc clean | 89946458... | MERGED 2026-09-14 |

...
| 2026-09-16 | security | Six `hive-mind_*` MCP tools (`spawn`, `consensus` `propose`, `broadcast`, `shutdown`, `memory` `set`/`delete`) mutated hive state with no `hiveToken` check, though sibling `join`/`leave`/`vote` were capability-bound the night before (#3291, merged 09-15) — most severely, `spawn` let an unauthenticated caller mint its own voting "workers" and vote as them, bypassing #3291 entirely via a sibling tool; all six now reuse the existing `requireHiveToken` gate | #3338 | #3339 | yes | ACCEPT | stash-isolated: 7/12 new tests fail on baseline exactly for the 6 gated call sites, 18/18 pass on candidate; full `@claude-flow/cli` suite (251 files/3165 tests) failing-test-name sets identical both ways (34 pre-existing, unrelated); `tsc --noEmit` byte-identical (22 errors); independent adversarial critique CONFIRMED-WITH-CAVEATS pre-fix (found the `hive-mind_memory` gap the first patch missed), re-verified after closing it | f2dca5c7... | 20/20 previously-checked backlog PRs now confirmed merged (see note above) — first night since 08-31 with zero open dream-cycle backlog |

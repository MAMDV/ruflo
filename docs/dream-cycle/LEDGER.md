...
| 2026-08-30 | performance | unwired `diskann-backend.ts` (375 lines, zero call sites, `@ruvector/diskann` not a dependency) removed; stale "150x-12,500x faster search" claim in generated CLAUDE.md replaced with a pointer to `benchmark-intelligence.mjs --only=hnsw` instead of a hardcoded multiplier | #3129 | #3130 | yes | ACCEPT | baseline (git-stash) fails predicted 17/17 `generateClaudeMd` regression test; candidate passes; full CLI suite byte-identical pre-existing failure list vs controlled baseline (environmental, missing native `@ruvector/*-wasm`) | c02c9e7f... | rebased onto main (post #3086/#3094/#3103/#3110/#3119/#3123 merges), awaiting fresh CI as of 2026-09-14 |
